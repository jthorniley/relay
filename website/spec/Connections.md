...
HasPreviousPage(allEdges, before, after, first, last):
  ...
  * If {after} is set:
    ...
    * If edges exist before the edge corresponding to {after}, then optionally
      return {true} if it is efficient for the server to find those edges.
  * Return {false}.
...
HasNextPage(allEdges, before, after, first, last):
  ...
  * If {before} is set:
    ...
    * If edges exist after the edge corresponding to {before}, then optionally
      return {true} if it is efficient for the server to find those edges.
  * Return {false}.
